...
func parseArgs() error {
	...
	// Default comes from config (and NewConfig). Only override if explicitly set on CLI.
	netTitleStats := conf.NetTitleStats
	netTitleStatsSet := false
	goopt.NoArg([]string{"--net-title-stats"}, tr.Value("args.net-title-stats"), func() error {
		netTitleStats = true
		netTitleStatsSet = true
		return nil
	})
	goopt.NoArg([]string{"--no-net-title-stats"}, tr.Value("args.no-net-title-stats"), func() error {
		netTitleStats = false
		netTitleStatsSet = true
		return nil
	})
